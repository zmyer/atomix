...
public class DefaultPartitionService implements ManagedPartitionService {
        ...
        return groupMembershipService.start()
                .thenCompose(v -> {
                    ...
                    if (systemGroupMembership != null) {
                        if (systemGroup == null) {
                            systemGroup = systemGroupMembership.config().getType()
                        ...
                        electionService = new HashBasedPrimaryElectionService(clusterMembershipService,
                                groupMembershipService, communicationService);
                        return electionService.start()
                                .thenCompose(s -> {
                                    ...
                                    if (systemGroupMembership.members().contains(
    ...
    public CompletableFuture<Void> stop() {
        ...
        Stream<CompletableFuture<Void>> systemStream = Stream.of(
        ...
        return CompletableFuture.allOf(futures.toArray(new CompletableFuture[futures.size()])).exceptionally(
                throwable -> {
                    LOGGER.error("Failed closing partition group(s)", throwable);
                    return null;
                }).thenCompose(
                v -> electionService != null ? electionService.stop() : CompletableFuture.completedFuture(null))
