...
public class RaftSessionConnection {
    ...
    public RaftSessionConnection(RaftClientProtocol protocol, MemberSelector selector,
                                 ThreadContext context, LoggerContext loggerContext) {
     ...
     */
    protected <T extends RaftRequest, U extends RaftResponse>
     ...
     */
    protected <T extends RaftRequest, U extends RaftResponse>
    void sendRequest(T request, BiFunction<MemberId, T,
            CompletableFuture<U>> sender, int count, CompletableFuture<U> future) {
        MemberId node = next();
        if (node != null) {
