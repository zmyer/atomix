...
public class PrimaryBackupSessionClient implements SessionClient {
    ...
    private void execute(PrimitiveOperation operation, int attempt, ComposableFuture<byte[]> future) {
        ...
        final ExecuteRequest request = ExecuteRequest.request(descriptor, sessionId.id(),
                clusterMembershipService.getLocalMember().id(), operation);
        log.trace("Sending {} to {}", request, term.primary());
        final PrimaryTerm term = this.term;
        if (term.primary() != null) {
                        ...
                        primaryElection.getTerm().whenComplete((newTerm, termError) -> {
                            if (termError == null) {
                                if (newTerm.term() > term.term() && newTerm.primary() != null) {
                                    ...
                                    threadContext.schedule(Duration.ofMillis(RETRY_DELAY),
                                            () -> execute(operation, attempt + 1, future));
                                }
                            } else {
                                final Throwable cause = Throwables.getRootCause(termError);
