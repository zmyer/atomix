...
public class DefaultConsistentMapService
    ...
    public void restore(BackupInput reader) {
        ...
        map.forEach((key, value) -> {
            if (value.ttl() > 0) {
                value.timer = getScheduler().schedule(
                        Duration.ofMillis(value.ttl() - (getWallClock().getTime().unixTimestamp() - value.created())),
                        () -> {
    ...
    public MapEntryUpdateResult<String, byte[]> remove(String key, byte[] value) {
        return removeIf(getCurrentIndex(), key, v ->
                valuesEqual(v, new MapEntryValue(MapEntryValue.Type.VALUE, getCurrentIndex(), value,
